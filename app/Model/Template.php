...
class Template extends Model
{
    ...
    protected $fillable = ['template_title', 'english', 'arabic', 'for', 'status'];
    public $timestamps = false;

    public function add($data) {

        $data['template_title']['en'] = cleanNameString($data['template_title']['en']);

		if (!isset($data['template_title']['en']) || $data['template_title']['en'] == '') {
			Error::trigger("template.add", ["Please Enter title in English. Special Characters are not allowed."]);
			return false;
		}

        $data['template_title']['ar'] = cleanNameString($data['template_title']['ar']);

		if (!isset($data['template_title']['ar']) || $data['template_title']['ar'] == '') {
			Error::trigger("template.add", ["Please Enter title in Arabic. Special Characters are not allowed."]);
			return false;
		}

        $data['template_title'] = json_encode($data['template_title'], JSON_UNESCAPED_UNICODE);

        try {

            if (isset($data['subject'])) {
                $data['subject'] = json_encode($data['subject'], JSON_UNESCAPED_UNICODE);
            }
    ...
    public function change($data, $id) {
        try {
            ...
            $template = parent::change($data, $id);
            return $template;
        } catch(\Exception $ex) {
    ...
    /**
     * In Fleet management, Fleet Events could be defined related to Fleet, e.g Oil Change, Car Wash, etc.
     * all events will be saved in `fm_events` table in db and all vehicle and vehicle groups will be attached to it upon which
     * this event will occur.
     * When Event time occurs notifictaions will be send on SMS, Email (if enabled) to user(driver).
    */
    public static function sendFleetEventNotifications($event_id) {
        if (!isset($event_id) || $event_id == 0) {
            return [
                "code" => 400,
                "message" => "Fleet Event not found, Notification can not be sent."
            ];
        }

        $event = \App\Model\Event::with('vehicles', 'vehicleGroups')->find($event_id);
    ...
    // public static function sendNotificationsToReps($order_id, $order_status_id = 0) {
